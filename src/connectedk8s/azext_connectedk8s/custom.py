...
# !!PDS: The equivalent of powershell New.
def create_connectedk8s(
    ...
    # !!PDS: Strange - isn't this read from the DP?
    ...
    configuration_settings, configuration_protected_settings, protected_helm_values = (
        utils.add_config_protected_settings(
            http_proxy,
            https_proxy,
            no_proxy,
            proxy_cert,
    ...
    if (
        ...
        arc_agent_configurations = generate_arc_agent_configuration(
    ...
    if enable_gateway:
        ...
        if re.match(gateway_armid_pattern, gateway_resource_id):
            ...
            raise InvalidArgumentValueError(
                str.format(consts.Gateway_ArmId_Is_Invalid, gateway_resource_id)
            )
    ...
    try:
        ...
        telemetry.set_exception(
            exception="An exception has occured while trying to execute pre-onboarding diagnostic \
            checks : {}".format(
                str(e)
            ),
    ...
    if diagnostic_checks != consts.Diagnostic_Check_Passed and lowbandwidth is False:
        ...
        if diagnostic_checks == consts.Diagnostic_Check_Incomplete:
            ...
            raise ValidationError(
                "Execution of pre-onboarding checks failed and hence not proceeding with cluster \
                onboarding. Please meet the prerequisites \
                    - "
                + consts.Onboarding_PreRequisites_Url
    ...
    if lowbandwidth is False:
        ...
        print(
            "Skipped onboarding pre-checks for AKS-HCI low bandwidth scenario. Continuing..."
        )
    ...
    if not required_node_exists:
        ...
        logger.warning(
            "Please ensure that this Kubernetes cluster has any nodes with OS 'linux', for scheduling the Arc-Agents onto and connecting to Azure. "
            "Learn more at %s",
    ...
    if not crb_permission:
        telemetry.set_exception(
            exception="Your credentials doesn't have permission to create clusterrolebindings on \
    ...
    if enable_private_link is True:
        try:
            ...
            if pls_location != location.lower():
                telemetry.set_exception(
                    exception="Connected cluster resource and Private link scope resource are \
    ...
    if release_namespace:
        ...
        try:
            ...
            utils.kubernetes_exception_handler(
                ...
                message_for_not_found="The helm release 'azure-arc' is present but the \
                                                    ...
                                                    before onboarding the cluster again.".format(
                    release_namespace
                ),
            )
        ...
        if connected_cluster_exists(client, configmap_rg_name, configmap_cluster_name):
            ...
            if (
                ...
                # !!PDS: Not clear why this is here and we are not just rejecting this request.
                #        Copilot suggests giving the user the option of what happens next!
            ...
            # !!PDS: Note exit point here having updated the settings but
            #        apparently not updated the cluster itself (assumed unchanged?).
            ...
            # !!PDS: This might explain the Powershell operation, but really?
    else:
    ...
    # !!PDS: Above here seems to be where we have been writing the Azure objects
    #        and then we call the DP.
...
def validate_existing_provisioned_cluster_for_reput(
    ...
    if (
        ...
        if azure_hybrid_benefit is not None:
            raise InvalidArgumentValueError(
                "Updating the 'azure hybrid benefit' property of a Provisioned Cluster \
        ...
        for value in validation_values:
            if value is not None:
                raise InvalidArgumentValueError(
                    "Updating the following properties of a Provisioned Cluster are not supported from the Connected Cluster CLI: kubernetes_distro, kubernetes_infra, enable_private_link, private_link_scope_resource_id, distribution_version, azure_hybrid_benefit, location, public_key.\n\nPlease use the 'az aksarc update' CLI command. https://learn.microsoft.com/en-us/cli/azure/aksarc?view=azure-cli-latest#az-aksarc-update"
                )
...
def validate_env_file_dogfood(values_file):
    ...
    with open(values_file, "r") as f:
        ...
        try:
            ...
            raise FileOperationError(
                "The required helm environment variables for dogfood onboarding are either not present in the file or incorrectly set.",
    ...
    if not os.path.isfile(install_location):
        ...
        try:
            ...
            raise ClientRequestError(
                ...
                recommendation="Please ensure that you delete the directory '{}' before trying again.".format(
                    download_dir
                ),
            )
        # Extract the archive.
...
def delete_connectedk8s(
    ...
    if not force_delete:
        ...
        confirmation_message = (
            "Force delete will clean up all the azure-arc resources, including extensions. Please make sure your current kubeconfig is pointing to the right cluster.\n"
    ...
    try:
        ...
        utils.kubernetes_exception_handler(
            ...
            message_for_not_found="The helm release 'azure-arc' is present but the azure-arc namespace/configmap is missing. Please run 'helm delete azure-arc --namepace {} --no-hooks' to cleanup the release before onboarding the cluster again.".format(
                release_namespace
            ),
        )
...
def update_connected_cluster(
    ...
    # !!PDS: We have to assume that this is going to become a GET/PUT.
    ...
    if auto_upgrade is not None:
        cmd_helm_upgrade.extend(
            [
                "--set",
                "systemDefaultValues.azureArcAgents.autoUpdate={}".format(auto_upgrade),
            ]
        )
    if https_proxy:
    ...
    if container_log_path is not None:
        cmd_helm_upgrade.extend(
            [
                "--set",
                "systemDefaultValues.fluent-bit.containerLogPath={}".format(
                    container_log_path
                ),
            ]
        )
    ...
    if release_namespace:
        ...
        try:
            ...
            utils.kubernetes_exception_handler(
                ...
                "Unable to read ConfigMap",
                error_message="Unable to read ConfigMap 'azure-clusterconfig' in 'azure-arc' namespace: ",
                message_for_not_found="The helm release 'azure-arc' is present but the azure-arc namespace/configmap is missing. Please run 'helm delete azure-arc --namespace {} --no-hooks' to cleanup the release before onboarding the cluster again.".format(
                    release_namespace
                ),
            )
        configmap_rg_name = configmap.data["AZURE_RESOURCE_GROUP"]
        ...
        if connected_cluster_exists(client, configmap_rg_name, configmap_cluster_name):
            if not (
                ...
                raise ArgumentUsageError(
                    "The provided cluster name and resource group name do not correspond to the kubernetes cluster you are trying to upgrade.",
        ...
        if auto_update_enabled == "true":
            ...
            raise ClientRequestError(
                "az connectedk8s upgrade to manually upgrade agents and extensions is only supported when auto-upgrade is set to false.",
        ...
        raise ClientRequestError(
            "The azure-arc release namespace couldn't be retrieved, which implies that the kubernetes cluster has not been onboarded to azure-arc.",
    ...
    if release_namespace:
        ...
        if connected_cluster_exists(client, configmap_rg_name, configmap_cluster_name):
            if not (
                ...
                raise ArgumentUsageError(
                    "The provided cluster name and resource group name do not correspond to the kubernetes cluster you are operating on.",
...
def disable_features(
    ...
    features = [x.lower() for x in features]
    confirmation_message = (
        "Disabling few of the features may adversely impact dependent resources. Learn more about this at https://aka.ms/ArcK8sDependentResources. \n"
    ...
    disable_cluster_connect, disable_azure_rbac, disable_cl = (
        utils.check_features_to_update(features)
    )
    ...
    if disable_cluster_connect:
        try:
            ...
            if (
                not disable_cl
                and helm_values.get("systemDefaultValues")
                .get("customLocations")
                .get("enabled")
                is True
                and helm_values.get("systemDefaultValues")
...
def client_side_proxy(
    ...
    if flag == 0:
        ...
        ):  # refresh token approach if cli is using ADAL auth. This is for cli < 2.30.0
            if user_type == "user":
                identity_data = {}
    ...
    if flag == 0:
        ...
        try:
            ...
            if path != "-":
                ...
                print(
                    "Start sending kubectl requests on '{}' context using kubeconfig at {}".format(
...
def check_cl_registration_and_get_oid(cmd, cl_oid, subscription_id):
    ...
    try:
        ...
        enable_custom_locations = False
        logger.warning(
            "Unable to fetch registration state of 'Microsoft.ExtendedLocation'. Failed to enable 'custom-locations' feature. This is fine if not required. Proceeding with helm install."
        )
        telemetry.set_exception(
            exception=e,
    ...
    try:
        ...
        if len(app_object) != 0:
            ...
            if cl_oid is not None and cl_oid != app_object[0]["id"]:
                logger.debug(
                    "The 'Custom-locations' OID passed is different from the actual OID(%s) of the Custom Locations RP app. Proceeding with the correct one...",
                    app_object[0]["id"],
                )
        ...
        if cl_oid is None:
            logger.warning(
                "Failed to enable Custom Locations feature on the cluster. Unable to fetch Object ID of Azure AD application used by Azure Arc service. Try enabling the feature by passing the --custom-locations-oid parameter directly. Learn more at https://aka.ms/CustomLocationsObjectID"
            )
            telemetry.set_exception(
...
def check_operation_support(operation_name, agent_version):
    ...
    if "-" not in agent_version and (
        ...
        raise ClientRequestError(
            error_summary,
            recommendation="Please upgrade to the latest version of the Agents using 'az connectedk8s upgrade -g <rg_name> -n <cluster_name>'.",
        )
